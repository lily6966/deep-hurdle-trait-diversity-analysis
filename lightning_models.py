...
class DeepHurdleModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        Returns:
            Loss dictionary
        """
        loc, features, orig_labels, count_labels, binary_labels = batch
        ...
        if not self.classifier_trained:
            # Train classifier
            output = self.classifier(features, count_labels, binary_labels)
            ...
            # Update metrics
            pred_binary_rounded = (pred_x > 0.5).float()
            ...
            with torch.no_grad():
                classifier_output = self.classifier(
                    features, count_labels, binary_labels
                )
                pred_binary = torch.sigmoid(classifier_output["feat_out"])

            # Forward pass through regressor
            output = self.regressor(features, count_labels, binary_labels)
            total_loss, nll_loss, nll_loss_x, kl_loss, cpc_loss, pred_e, pred_x = (
                compute_loss(
                    binary_labels,
                    count_labels,
            ...
            pred_x_real = torch.expm1(adjusted_pred_x)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        classifier_output = self.classifier(features, count_labels, binary_labels)
        regressor_output = self.regressor(features, count_labels, binary_labels)
        ...
        pred_binary_rounded = (pred_x_classifier > 0.5).float()
 ...
 #       # Store locations for later use
        self.test_loc.append(loc.cpu().numpy())  # Assuming first two columns are locations
    def on_predict_epoch_end(self):
        ...
        binary_pred = np.concatenate(self.test_binary_pred)
        count_pred = np.concatenate(self.test_count_pred)
        loc_pred = np.concatenate(self.test_loc)  
        ...
        data_file = np.load('./DATA/ebird_entire/ebird_data_withloc.npz', allow_pickle=True)
        ...
        binary_csv_path = os.path.join(log_dir, "binary_pred_results.csv")
